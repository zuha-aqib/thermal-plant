...
- Continuously estimate the current thermal scale with background RapidOCR/OpenVINO.
...
This script uses RapidOCR/OpenVINO for live MIN/MAX recognition and reuses
Step-03 temperature conversion/masking logic so hover, ROI statistics and
hotspots remain mathematically consistent with the recorded-video pipeline.
...
# ---------------------------------------------------------------------------
# RapidOCR backend used for live thermal-scale OCR.
# Mirrors the compatibility shim used in the teammate implementation.
# ---------------------------------------------------------------------------
...
def get_rapidocr_reader():
    """Create one reusable RapidOCR/OpenVINO reader lazily."""
    global _RAPIDOCR_READER

    if RapidOCR is None:
        details = (
            f"\\nImport error: {RAPIDOCR_IMPORT_ERROR}"
            if RAPIDOCR_IMPORT_ERROR is not None
            else ""
        )

        raise RuntimeError(
            "RapidOCR/OpenVINO is required for live thermal-scale OCR.\\n\\n"
            "Install the same backend used by the teammate code:\\n"
            "  py -3.12 -m pip install openvino rapidocr-openvino"
            + details
        )

    if _RAPIDOCR_READER is None:
        print("Initializing RapidOCR/OpenVINO...")
        _RAPIDOCR_READER = RapidOCR()
        print("RapidOCR/OpenVINO ready.")
...
# =============================================================================
# RAPIDOCR THERMAL-LABEL READING
# =============================================================================

def _rapidocr_result_items(result):
    """
    Normalize RapidOCR output into text/confidence pairs.

    The teammate implementation receives detections shaped approximately as:
        [box, text, score]
    """
    items = []
    ...
    for detection in result:
        try:
            if isinstance(detection, (list, tuple)) and len(detection) >= 2:
                text_value = str(detection[1]).strip()
                confidence = 0.0
...
def rapidocr_temperature(
    ...
    *,
    ...
    """
    Read one thermal MIN/MAX label with RapidOCR/OpenVINO.

    Returns:
        value, raw_text, confidence

    Uses the safer Step-03 parser:
      - '469' can become 46.9 for one decimal place.
      - tiny partial reads such as '8' or '15' are rejected.
    """
    reader = get_rapidocr_reader()

    crop = t3.crop_roi_with_padding(
        frame,
        roi,
    ...
    scale_factor = max(1.0, float(upscale))
    ...
    result, _ = reader(crop_for_ocr)
    items = _rapidocr_result_items(result)
    ...
    if len(items) > 1:
        ...
        combined_value = t3.parse_temperature(
            combined_text,
            decimal_places=int(decimal_places),
        )

        if np.isfinite(combined_value):
            candidates.append(
                {
...
def rapidocr_scale_pair(
    frame: np.ndarray,
    scale: dict,
    args,
):
    """Read and physically validate live MIN/MAX with RapidOCR."""
    decimal_places = int(
        scale.get(
    ...
    min_value, min_text, min_confidence = rapidocr_temperature(
        frame,
        scale["min_text_roi"],
        decimal_places=decimal_places,
        padding=padding,
        upscale=float(args.rapidocr_upscale),
        min_confidence=float(args.rapidocr_min_confidence),
    )

    max_value, max_text, max_confidence = rapidocr_temperature(
        frame,
        scale["max_text_roi"],
        decimal_places=decimal_places,
        padding=padding,
    ...
    if not t3._basic_scale_pair_valid(
        min_value=min_value,
        max_value=max_value,
...
def configure_scale_from_frame(
    ...
    """First-time thermal-scale setup using RapidOCR/OpenVINO."""
    get_rapidocr_reader()

    print("\n============================================================")
    print("FIRST-TIME STREAM SETUP: THERMAL SCALE")
    print("OCR ENGINE: RAPIDOCR + OPENVINO")
    print("============================================================")
    ...
    while True:
        ...
        result = rapidocr_scale_pair(
            frame,
            provisional_scale,
            args,
        )

        debug_dir = preset_dir / "rapidocr-debug"
        debug_dir.mkdir(
            parents=True,
            exist_ok=True,
        )
        ...
        cv2.imwrite(
            str(debug_dir / "MAX_selected_crop.png"),
            t3.crop_roi_with_padding(
                frame,
                max_roi,
        ...
        cv2.imwrite(
            str(debug_dir / "MIN_selected_crop.png"),
            t3.crop_roi_with_padding(
                frame,
                min_roi,
                padding=int(args.ocr_padding),
        ...
        if result is None:
            min_value, min_text, min_conf = rapidocr_temperature(
                frame,
                min_roi,
            ...
            max_value, max_text, max_conf = rapidocr_temperature(
                frame,
                max_roi,
                decimal_places=int(args.decimal_places),
                padding=int(args.ocr_padding),
            ...
            print("\nRapidOCR could not validate this scale pair.")
            print(
                f"  MIN raw: {min_text!r} | "
                f"value: {min_value if np.isfinite(min_value) else 'FAIL'} | "
                f"confidence: {min_conf:.3f}"
            )
            print(
                f"  MAX raw: {max_text!r} | "
                f"value: {max_value if np.isfinite(max_value) else 'FAIL'} | "
        ...
        print("\nRAPIDOCR SCALE VALIDATION")
        print("-" * 56)
        print(
            f"MIN: {result['min_temp']:.1f} C | "
            f"raw {result['min_text']!r} | "
            f"confidence {result['min_confidence']:.3f}"
        )
        print(
            f"MAX: {result['max_temp']:.1f} C | "
            f"raw {result['max_text']!r} | "
            f"confidence {result['max_confidence']:.3f}"
        )
        print(
            f"Span: "
            f"{result['max_temp'] - result['min_temp']:.1f} C"
        )

        if not prompt_yes_no(
            "Use this RapidOCR thermal scale setup?",
            default=True,
        ):
            continue
        ...
        config = {
            ...
            "scale": {
                ...
                "ocr_engine": "rapidocr_openvino",
                "rapidocr_upscale": float(args.rapidocr_upscale),
                "rapidocr_min_confidence": float(
                    args.rapidocr_min_confidence
                ),
                ...
                "reference_ocr_min_confidence": float(
                    result["min_confidence"]
                ),
                "reference_ocr_max_confidence": float(
...
class LiveScaleTracker:
    """Background RapidOCR scale tracker with temporal safety checks."""
    ...
    def __init__(
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix="rapidocr-live-scale",
        )
        ...
        if preset_engine == "rapidocr_openvino":
            ...
            self.status = (
                "RAPIDOCR REFERENCE SCALE"
                if (
                    self.trusted_min is not None
                    and self.trusted_max is not None
                )
                else "WAITING FOR RAPIDOCR"
            )
        else:
            # Reuse old annotation/scale ROIs, but do NOT reuse a possibly
            # incorrect Tesseract-derived temperature seed.
            self.trusted_min = None
            self.trusted_max = None
            self.status = (
                "LEGACY PRESET - WAITING FOR FRESH RAPIDOCR"
            )

        self.pending_jump: Optional[
            Tuple[float, float]
        ] = None
...
def build_parser():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "RapidOCR/OpenVINO is now used instead of Tesseract."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.25,
        help=(
            "Reject RapidOCR detections below this confidence. "
            "Default: 0.25"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=2.0,
        help=(
            "Upscale selected MIN/MAX crops before RapidOCR. "
            "Default: 2.0"
        ),
    )
...
def main():
    ...
    print("OCR engine : RapidOCR + OpenVINO")
    print(
        f"OCR config : confidence >= "
        f"{args.rapidocr_min_confidence:.2f}, "
        f"upscale {args.rapidocr_upscale:.1f}x"
    )
